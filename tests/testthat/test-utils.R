...
test_that("Directory creation with date", {
  ...
  create_directory(dir_prefix, date = TRUE)
...
test_that("Handling existing directory", {
  ...
  create_directory(dir_name)
  create_directory(dir_name)  # Attempt to create again
...
test_that("Warning on failed directory creation", {
  ...
  expect_warning(create_directory(dir_name), "Failed to create directory")
  unlink(dir_name, recursive = TRUE)
})
